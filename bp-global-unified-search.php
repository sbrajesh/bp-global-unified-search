...
class BPUnifiedsearch {
	...
	private function __construct() {
		...
		add_filter( 'bp_search_form_action', array( $this, 'get_search_url' ) );
	}
	...
	//add root component
	public function setup_search_component() {
	...
	//remove from nav
	public function remove_from_nav( $excluded ) {
	...
	public function get_title( $title, $cur_title, $sep, $seplocation ) {
		global $bp;
	...
	public function get_search_url( $current_url ) {
		global $bp;
		
		$page_id = $bp->pages->search->id;
		if ( empty( $page_id ) )
			return $current_url;
